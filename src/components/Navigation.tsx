    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? 'bg-[#f1f0ee]/95 backdrop-blur-md border-b border-black/5'
            : 'bg-transparent'
        }`}
      ...
      >
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="flex items-center justify-between h-20">
            ...
            >
              {menuOpen ? (
                <X className={scrolled ? 'text-black' : 'text-white'} size={24} />
              ) : (
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-40 bg-[#0f2e4a] transition-transform duration-500 md:hidden ${
        ...
        <div className="flex flex-col items-center justify-center h-full gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="text-2xl font-light text-white hover:text-[#ff6600] transition-colors"
